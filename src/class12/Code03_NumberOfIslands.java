...
public class Code03_NumberOfIslands {
	...
	public static void main(String[] args) {
		...
		start = System.currentTimeMillis();
		System.out.println("并查集(map实现)的运行结果: " + numIslands4(board2));
		end = System.currentTimeMillis();
	...
	public static int numIslands4(char[][] board) {
		int row = board.length;
		int col = board[0].length;
		// board数组等价平移为dots数组
		Dot[][] dots = new Dot[row][col];
		...
		UnionFind4<Dot> uf = new UnionFind4<>(dotList);
		// 第0行考察左边是否和自己相等
		for (int j = 1; j < col; j++) {
			// (0,j)  (0,0)跳过了  (0,1) (0,2) (0,3)
			if (board[0][j - 1] == '1' && board[0][j] == '1') {
	...
	public static class UnionFind4<V> {

		public HashMap<V, V> parents;
		public HashMap<V, Integer> sizeMap;

		public UnionFind4(List<V> values) {
			parents = new HashMap<>();
			sizeMap = new HashMap<>();
			for (V cur : values) {
				parents.put(cur, cur);
		...
		public V findFather(V cur) {
			LinkedList<V> deQueue = new LinkedList<>();
			while (cur != parents.get(cur)) {
				deQueue.addFirst(cur);
				cur = parents.get(cur);
			}
			while (!deQueue.isEmpty()) {
				parents.put(deQueue.pollFirst(), cur);
			}
			return cur;
		}
