...
public class Code03_CoinsWayNoLimit {
	...
	private static int processs(int[] arr, int index, int rest) {
		...
		for (int i = 0; i <= rest/arr[index]; i++) {
			ans += processs(arr, index+1, rest-(arr[index]*i));
		}
		return ans;
	}

	public static int coinsWayyy(int[] arr, int aim) {
		if (arr == null || arr.length == 0 || aim < 0) {
			return 0;
		}
		int N = arr.length;
	...
	public static int process(int[] arr, int index, int rest) {
		...
		int ways = 0;
		// zhang * arr[index] <= rest 限制了rest不会小于0
		for (int zhang = 0; zhang * arr[index] <= rest; zhang++) {
			// 当前货币用0张，1张，2张，3张...，但不能超过目标rest钱数
